void draw_npe_mom()
{
    ...
    tt1->Draw("BASHIPHInfoCol.npe:sqrt(pow(BASHIPHBarHitCol.momentum.x[0],2) + pow(BASHIPHBarHitCol.momentum.y[0],2) + pow(BASHIPHBarHitCol.momentum.z[0],2) ) >> pr1", "BASHIPHBarHitCol.cntID[0]==0");

    TChain *tt2=new TChain("events");	
    TString infilepi = pref + "/" + "sim_pi-.root";
    tt2->Add(infilepi);
    tt2->Draw("BASHIPHInfoCol.npe:sqrt(pow(BASHIPHBarHitCol.momentum.x[0],2) + pow(BASHIPHBarHitCol.momentum.y[0],2) + pow(BASHIPHBarHitCol.momentum.z[0],2) ) >> pr2", "BASHIPHBarHitCol.cntID[0]==0");

    TChain *tt3=new TChain("events");	
    TString infilek = pref + "/" + "sim_kaon-.root";
    tt3->Add(infilek);
    tt3->Draw("BASHIPHInfoCol.npe:sqrt(pow(BASHIPHBarHitCol.momentum.x[0],2) + pow(BASHIPHBarHitCol.momentum.y[0],2) + pow(BASHIPHBarHitCol.momentum.z[0],2) ) >> pr3", "BASHIPHBarHitCol.cntID[0]==0");
    ...
    TLegend* leg=new TLegend(100,3.0,350,6,"","");
